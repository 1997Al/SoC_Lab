...
def curve_fitting_func(x_data, y_data, model_type, p0):
    x = np.array(x_data)
    y = np.array(y_data)
...
def computation_module(approximate_adder, bit_width: int):
    ...
    error_rate_model_parameter = curve_fitting_func(approximation_degrees, error_rate, 'error_rate_model',
                                                    p0=[1.0, 1.0, 1.0])
    mean_error_model_parameter = curve_fitting_func(approximation_degrees, mean_error, 'mean_error_model',
                                                   p0=[1, 1, 0.693])
